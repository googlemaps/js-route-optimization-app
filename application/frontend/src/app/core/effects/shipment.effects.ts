...
export class ShipmentEffects {
  confirmDelete$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ShipmentActions.confirmDeleteShipment),
      exhaustMap((action) =>
          ...
          mergeMap((hasSolution) => {
            const dialogRef = this.dialog.open(ConfirmDialogComponent, {
              data: {
                ...
                message:
                  `Delete Shipment #${action.id}?` +
                  (hasSolution ? ' This will also clear the current solution.' : ''),
              },
            });
            return dialogRef.afterClosed().pipe(
              filter((res) => res),
              map(() => ShipmentActions.deleteShipment({ id: action.id }))
            );
          })
        )
      )
  ...
  confirmBulkDelete$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ShipmentActions.confirmDeleteShipments),
      exhaustMap((action) =>
        this.store.select(selectHasSolution).pipe(
          take(1),
          mergeMap((hasSolution) => {
            const dialogRef = this.dialog.open(ConfirmDialogComponent, {
              data: {
                title: 'Confirm delete',
                message:
                  `Delete ${action.ids.length} shipments?` +
                  (hasSolution ? ' This will also clear the current solution.' : ''),
              },
            });
            return dialogRef.afterClosed().pipe(
              filter((res) => res),
              map(() => ShipmentActions.deleteShipments({ ids: action.ids }))
            );
          })
        )
      )
